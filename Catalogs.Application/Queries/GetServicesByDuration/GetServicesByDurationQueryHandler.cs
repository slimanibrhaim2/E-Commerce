        ...
        try
        {
            if (request.MinDuration > request.MaxDuration)
                return Result<PaginatedResult<ServiceDTO>>.Fail(
                    message: "يجب أن تكون المدة الدنيا أقل من المدة القصوى",
            ...
            var paginatedServices = await _repo.GetByDurationRange(request.MinDuration, request.MaxDuration, request.PageNumber, request.PageSize);
            ...
            var serviceDTOs = paginatedServices.Data.Select(service => new ServiceDTO
            {
                Id = service.Id,
                Name = service.Name,
