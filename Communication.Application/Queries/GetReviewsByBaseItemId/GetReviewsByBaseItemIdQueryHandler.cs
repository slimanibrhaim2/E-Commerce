...
public class GetReviewsByBaseItemIdQueryHandler : IRequestHandler<GetReviewsByBaseItemIdQuery, Result<PaginatedResult<ReviewDTO>>>
{
    private readonly IReviewRepository _reviewRepository;

    public GetReviewsByBaseItemIdQueryHandler(IReviewRepository reviewRepository)
    {
        _reviewRepository = reviewRepository;
    }

    public async Task<Result<PaginatedResult<ReviewDTO>>> Handle(GetReviewsByBaseItemIdQuery request, CancellationToken cancellationToken)
    {
        try
        {
            var reviews = await _reviewRepository.GetAllByBaseItemIdAsync(request.BaseItemId);
        ...
        catch (Exception ex)
        {
            return Result<PaginatedResult<ReviewDTO>>.Fail(
                message: $"فشل في جلب مراجعات العنصر: {ex.Message}",
